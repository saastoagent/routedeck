...
class RouteDeckStateProjector:
    ...
    def __init__(
        ...
        *,
        ...
        if resolved_manifest is None:
            raise TypeError("RouteDeckStateProjector requires a manifest or subclass manifest")
        ...
        if operation_review_component is not None:
            self.operation_review_component = operation_review_component
    ...
    def resolve_current_surface_id_for_state(self, state: RouteDeckGraphState) -> str | None:
    ...
    def default_surface_by_node_for_state(self, state: RouteDeckGraphState) -> dict[str, str]:
    ...
    def review_surface_props(self, state: RouteDeckGraphState, **context: Any) -> dict[str, Any]:
        return {}
    ...
    def surfaces_with_review(
        ...
        **context: Any,
        ...
        review_props = props if props is not None else context.get("review_surface_props")
        ...
        review_surface = self.surface_registry.operation_review_surface(
            ...
            component=component or self.operation_review_component,
            props=review_props,
        )
        for index, surface in enumerate(projected_surfaces):
            if surface.role == "active":
                return [*projected_surfaces[:index], review_surface, *projected_surfaces[index:]]
    ...
    def current_context_for_state(self, state: RouteDeckGraphState, **context: Any) -> str | None:
        current_context = context.get("current_context")
        return str(current_context) if current_context is not None else state.node

    def actions_for_state(self, state: RouteDeckGraphState, **context: Any) -> Sequence[Any]:
        actions = context.get("actions")
        return list(actions) if isinstance(actions, Sequence) else []

    def surfaces_for_state(self, state: RouteDeckGraphState, **context: Any) -> Sequence[RouteDeckSurface]:
        surfaces = context.get("surfaces")
        return list(surfaces) if isinstance(surfaces, Sequence) else []

    def presentation_state_for_state(self, state: RouteDeckGraphState, **context: Any) -> Mapping[str, Any]:
        presentation_state = context.get("presentation_state")
        return dict(presentation_state) if isinstance(presentation_state, Mapping) else {}

    def diagnostics_for_state(self, state: RouteDeckGraphState, **context: Any) -> Mapping[str, Any]:
        diagnostics = context.get("diagnostics")
        return dict(diagnostics) if isinstance(diagnostics, Mapping) else {}

    def context_lens_for_state(self, state: RouteDeckGraphState, **context: Any) -> RouteDeckContextLens | Mapping[str, Any] | None:
        context_lens = context.get("context_lens")
        if context_lens is None:
            return None
        if isinstance(context_lens, RouteDeckContextLens) or isinstance(context_lens, Mapping):
            return context_lens
