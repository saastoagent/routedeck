...
| --- | --- | --- | --- | --- | --- |
| Operation invocation eligibility | Every Operation declares a required non-empty source set. Compiler validation rejects incompatible SurfaceAffordance, SuggestedAction, and RouteEntry bindings; public projection retains sorted source values; LangGraph exposes only agent-eligible legal tools; the runner blocks a disallowed source before providers, guards, review, or handler execution. | Operation/projection contracts, compiler validation, projector, supervised runner, LangGraph model context, and generated headless decoders. | Feature declarations select product-appropriate agent, surface, route, and system sources without moving product reasoning into RouteDeck. | RouteDeck reference; core-runtime and LangGraph component contracts; scoped-context and operation-supervision feature rows. | Focused compiler, projection/model-context, runner, generated-contract, and headless decoder tests. |
...
| QB-04 session/deployment ownership | Every generic HTTP plane requires `RouteDeckSessionSelector`; its request-aware created-session hook lets the host bind a principal/opaque handle or explicit guest cookie without moving auth policy into RouteDeck. Cookie/origin/worker/TTL/instance values remain host configuration. | `routedeck_fastapi/dependencies.py`, `session_http.py`, session route/runtime derivation. | Consumer host settings/runtime select identity or guest policy and perform any atomic owner-handle rebinding. | ADR-004 deployment boundary; ADR-006 transport boundary; generic HTTP and product-runtime feature rows. | `tests/fastapi/test_session_selection.py`, transport tests, consumer selector tests, schema-4 boundary report. |
